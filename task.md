...
## 現在地（2026-07-25）
- フェーズ: **実装ほぼ完了・検証待ち**。§0〜§6（共有契約 / ブラウザ対局 / PvC / PvP サーバ・ロビー / 鍵付き / 観戦 / 棋譜プレビュー / Web UX）は実装済みで、`npm run build` / `npm run lint` / Cloudflare smoke（Web 同士の PvP・鍵付き/観戦可否）まで緑。残る未完は **§7 Verification の4件と §8 Device-Dependent の4件に集中**。
- 進行中: 差異縮小の優先1「Web ローカル時計をモバイル既定（通常 3分+秒読み30秒 / インベーダー 15秒フェーズ）へ」は **2026-06-09 に完了**。以後コード変更なし（最終コミットは docs 系）。
- 次の一手:
  1. **§8 クロスデバイス PvP の実機確認**（Web 作成ルーム↔スマホ実機の相互参加、Web2ブラウザ+スマホ1台の混在 smoke）。
  2. §7 の全5バリアント PvC / PvP を開始から終局まで、公開ロビー観戦の着手同期、鍵付きロビーの拒否・参加。
  3. 棋譜 / `StoredResult` 差分を **MVP 制限事項として明文化**（完全互換は将来設計扱い）。
- ★ブロッカー/外部待ち: **Invader の PvP フェーズ同期が未対応**（本 task.md「2026-05-20 exshogi 差異確認メモ」）。Web は server message の `move` 単位で `turnSeat` を反転する前提が残るが、正は**フェーズ終了まで同じ手番を維持**する。**Web とスマホの混在 PvP には protocol / server 側の修正が必要**で、Web 単独では閉じられない＝上記1の前提。加えて §8 は全件が実機必須。
- 直近の重い判断: **仕様の正は `D:\dev\repos\exshogi-app`（モバイル版）**、Web はその追従実装。最重要の互換条件は「スマホアプリとブラウザが同一 PvP サーバ・同一ルーム・同一 protocol で対戦できること」。観戦リアクションと本将棋/Standard は Web では扱わない。
<!-- CURRENT-END -->
