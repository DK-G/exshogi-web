...
## 現在地（2026-08-02）
- フェーズ: **アーカイブ済み。このリポジトリでの開発は終了。**
- 移行先: `D:\dev\repos\exshogi-app\apps\web`（GitHub: `DK-G/EXshogi`）。Web 版のタスクは移行先の `apps/web/task.md` で管理する。
- 引き継ぎ: 本リポジトリ固有の実装（時計の状態別色分け）は移植済み。未コミット作業は各ブランチへ退避して push 済み。詳細は README.md 冒頭。
- ここより下は歴史的記録であり、更新しない。
<!-- CURRENT-END -->
